...
class MCTS:
    ...
    def __init__(self, exploration_weight=5000, policy='uct', budget=1000, optimum=0, n0=5, sigma_0=1):
        ...
        self.initial_n0 = n0
        self.n0 = n0
        self.sigma_prior = 1000000
        self.leaf_cnt = defaultdict(int)
        self.max_reward, self.min_reward = -100000, 100000
    ...
    def _select(self, node, path_reward):
        ...
        while True:
            ...
            self.n0 = max([2, self.initial_n0 - 2*node.stage])
...
#        print("start")
    ...
    def _backpropagate(self, path, sim_reward, path_reward):
        ...
        self.V_hat[leaf] = ((self.N[leaf]-1)/self.N[leaf])*self.V_hat[leaf] + sim_reward/self.N[leaf]
        for i in range(len(path)-2, -1, -1):
            ...
            if node.node_type == 's':
                ...
                # alpha = 1
                self.V_bar[node] = ((self.N[node]-1)/self.N[node])*self.V_bar[node] + self.ave_Q[child]/self.N[node]
                # if len(self.children[node]) == 0:
                #     print(node)
                self.V_hat[node] = (1-alpha) * self.V_bar[node] + alpha * max([self.ave_Q[c] for c in self.children[node] if self.N[c]>0])
            else:
                ...
                self.exploration_weight = max([self.exploration_weight, abs(r)])
    ...
    def _ocba_select(self, node):
        ...
        '''
        Method 2 to solve desiredReplications
